...
} from '../src/lib/workflowRecovery.ts';
...
test('restores the saved parking flow when offline and a stored workflow exists', () => {
  ...
  assert.equal(patch.stage, 'arrival');
...
test('only schedules background reminders when the workflow is ready and fewer than two reminders exist', () => {
  const workflowState = {
    stage: 'arrival',
  ...
  assert.equal(shouldScheduleReservationFollowUpNotifications(workflowState), true);
  assert.equal(
    shouldScheduleReservationFollowUpNotifications({
      ...workflowState,
...
test('blocks background reminders for the wrong workflow state', () => {
  ...
  for (const workflowState of notReadyStates) {
    assert.equal(shouldScheduleReservationFollowUpNotifications(workflowState), false);
  }
});
