...
export function Screen({ children, style, contentStyle, scrollEnabled = true }: ScreenProps) {
  ...
  const content = (
    <View style={[styles.inner, !scrollEnabled ? styles.innerFill : null, { paddingHorizontal: horizontalPadding }]}>
      <View style={[styles.maxWidth, !scrollEnabled ? styles.maxWidthFill : null, { maxWidth: contentWidth }, contentStyle]}>{children}</View>
    </View>
  );
