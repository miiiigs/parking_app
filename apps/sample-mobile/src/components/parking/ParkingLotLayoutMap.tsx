...
import { applyLiveSlotStatuses, buildRoadShape } from '../../../../../packages/shared/src/parkingMap';
  ...
  const panResponder = useMemo(
    () =>
      PanResponder.create({
        ...
        onPanResponderGrant: () => {
          startOffset.current = { x: (pan.x as unknown as { __getValue: () => number }).__getValue(), y: (pan.y as unknown as { __getValue: () => number }).__getValue() };
        },
  ...
  return (
    <View style={[styles.mapViewport, { minHeight: Math.max(420, screenHeight * 0.6) }]}>
      <View style={styles.mapHud} pointerEvents="box-none">
        ...
        <Text style={styles.mapHint}>Saved lot layout from admin. Drag to pan, tap an open slot to reserve.</Text>
      </View>
      ...
      <View style={styles.mapCanvasFrame} {...panResponder.panHandlers}>
        ...
        >
            ...
            {mergedLot.roads.map((road: ParkingMapRoad) => {
              ...
              return (
                <React.Fragment key={road.id}>
                  <Path d={shape.d} stroke="#0b1624" strokeWidth={shape.strokeWidth + 12} strokeLinecap="round" strokeLinejoin="round" opacity={0.8} fill="none" />
                  <Path d={shape.d} stroke="#17283d" strokeWidth={shape.strokeWidth + 4} strokeLinecap="round" strokeLinejoin="round" opacity={0.95} fill="none" />
                  <Path d={shape.d} stroke="#09111d" strokeWidth={shape.strokeWidth} strokeLinecap="round" strokeLinejoin="round" fill="none" />
                </React.Fragment>
              );
            })}
          </Svg>
          ...
          {mergedLot.nodes.map((node: ParkingMapNode) => (
            <View key={node.id} pointerEvents="none" style={[styles.entryPin, { left: node.x - 40, top: node.y }]}>
          ...
          {mergedLot.slots.map((slot: ParkingMapSlot) => {
            ...
            return (
              ...
              >
                ...
                <Text style={styles.mapSlotStatus}>{renderStatusLabel(slot.status)}</Text>
              </TouchableOpacity>
            );
          })}
        </Animated.View>
...
const styles = StyleSheet.create({
  mapViewport: { flex: 1, overflow: 'hidden', borderRadius: 18 },
