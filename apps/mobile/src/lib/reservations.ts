...
export type MobileProfileData = {
  ...
  recentActivity: MobileProfileActivityItem[];
...
export async function loadMobileProfileData(limit = 5): Promise<MobileProfileData | null> {
  const supabase = getSupabaseClient() as any;
  ...
  const recentActivity: MobileProfileActivityItem[] = [];
  ...
  for (const reservation of reservationRows) {
    ...
    const session = await getParkingSessionByReservationId(reservation.id).catch(() => null);

    if (session?.session_status === 'completed') {
      completedSessions += 1;
      totalBilledAmount += Number(session.billed_amount ?? session.reservation_fee ?? 0);
      recentActivity.push({
        id: `session-${session.session_id}`,
        title: 'Session completed',
        detail: `${parkingSlot?.slot_label ?? 'Assigned slot'} · Paid PHP ${Number(session.billed_amount ?? session.reservation_fee ?? 0).toFixed(2)}`,
    ...
    if (session) {
      ...
      recentActivity.push({
        ...
        detail: `${parkingSlot?.slot_label ?? 'Assigned slot'} · In progress`,
    ...
    recentActivity.push({
      ...
      title: reservation.status === 'confirmed' ? 'Reservation confirmed' : 'Reservation updated',
      detail: `${parkingSlot?.slot_label ?? 'Assigned slot'} · Expires ${formatProfileDate(reservation.expires_at)}`,
      timestamp: reservation.reserved_at,
      tone: reservation.status === 'confirmed' ? 'info' : 'neutral',
    });
  }

  return {
    displayName:
      currentUser.user_metadata?.full_name ?? currentUser.user_metadata?.name ?? currentUser.email?.split('@')[0] ?? 'Guest Driver',
    email: currentUser.email ?? null,
    memberSinceLabel: currentUser.created_at ? formatProfileDate(currentUser.created_at) : 'New member',
    plateNumber: reservationRows[0]?.plate_number ?? 'ABC-1234',
    stats: {
      reservationsCount: reservationRows.length,
      activeSessions,
      completedSessions,
      totalBilledAmount,
    },
